...
export async function POST(
  ...
  const review_notes = (body.review_notes ?? "").trim() || null;
  ...
  if (!["awaiting_review", "followup_received"].includes(oldAudit.status)) {
    return NextResponse.json(
      { error: `Cannot rerun from status "${oldAudit.status}"` },
      { status: 409 }
    );
  }
  ...
  if (payload) {
    fireRerunAuditWebhook(payload, newAudit.id).catch((err) =>
      console.error("[rerun] webhook error:", err)
    );
  }
